'use client'
import { useRouter } from 'next/navigation'
import HeadPage from "@/components/ui/HeadPage";
import {useHelpInfoModal} from "@/store";
import translations from "@/translations";
import {useLocale} from "@/context/locale";
import '@/sass/page.scss'


export default function Help() {
  const {locale} = useLocale()
  const router = useRouter()
  const {toggleModal, setData, setInData} = useHelpInfoModal()

  const helpVariants = [
    {
      id: 0,
      text: 'Не могу вернуться в Казахстан',
      textKk: 'Қазақстанға қайта алмаймын',
      ispPrevInfo: false
    },
    {
      id: 1,
      text: 'Не заселяют в отель',
      textKk: 'Қонақүйге қоныстырмайды',
      ispPrevInfo: false
    },
    {
      id: 2,
      text: 'Не могу вылететь из Казахстана',
      textKk: 'Қазақстаннан ұшып кете алмаймын',
      ispPrevInfo: false
    },
    {
      id: 3,
      text: 'Проблемы со здоровьем',
      textKk: 'Денсаулық мәселелері',
      ispPrevInfo: true,
      content: `<div class="modal__text-info">
        <ol>
          <li>
            <p>
              Незамедлительно лично или через представителя уведомить страховщика о наступлении страхового случая, и
              другие уполномоченные органы власти страны временного пребывания, и выполнять рекомендации п. 2-1 ст. 12
              Закона об обязательном страховании туриста (далее-Закон).
            </p>
            <div class="tooltip-wrap">
                 <div class="tooltip-btn" data-tooltip-id="my-tooltip" data-tooltip-variant="info" data-tooltip-place="right-start" data-tooltip-content="При получении документов на тур.поездку в обязательном порядке истребовать от исполнителя предоставление страхового сертификата и страхового полиса.">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 22C17.5 22 22 17.5 22 12C22 6.5 17.5 2 12 2C6.5 2 2 6.5 2 12C2 17.5 6.5 22 12 22Z" fill="white"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M12 7.25C12.4142 7.25 12.75 7.58579 12.75 8V13C12.75 13.4142 12.4142 13.75 12 13.75C11.5858 13.75 11.25 13.4142 11.25 13V8C11.25 7.58579 11.5858 7.25 12 7.25Z" fill="#D5251B"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M10.9945 16C10.9945 15.4477 11.4422 15 11.9945 15H12.0035C12.5558 15 13.0035 15.4477 13.0035 16C13.0035 16.5523 12.5558 17 12.0035 17H11.9945C11.4422 17 10.9945 16.5523 10.9945 16Z" fill="#D5251B"/>
                  </svg>
                </div>
            </div>
          </li>
          <li>
            <p>Представить страховщику имеющиеся документы, необходимые для выяснения обстоятельств о характере и
              размерах причиненного вреда здоровью (п. 2-1 ст.12 Закона), и в течении 5 рабочих дней страховщик должен
              определить размер страховой выплаты и предоставить на ознакомление (пп.4-3 п. 2 ст. 13 Закона). В том
              числе требовать возмещения расходов, понесённые в целях предотвращения или уменьшения убытков, если они
              были необходимы и были произведены для выполнения указаний страховщика, независимо от наступившего
              результата (п.3 ст. 14 Закона).

              Перечень необходимых документов : документ удостоверяющий личность, или паспорт застрахованного с отметкой
              пограничной службы; оригинал или нотариально заверенная копия медицинского документа, платежные документы
              об оплате медицинских услуг, оплаты товаров и услуг (приобретение лекарственных препараторов и иных
              медицинских услуг), медицинские документы касательно употребления психоактивного вещества и состоянии
              опьянения застрахованного при наступлении несчастного случая;
              Документы, подтверждающие аннулирование или обмен проездных документов, отказ от забронированного
              гостиничного номера, и других услуг.
              При транспортировке несовершеннолетних документы, подтверждающие осуществление телефонных звонков,
              сообщений страховщику; документы связанные с принятием мер по предотвращению и уменьшению убытков;
              документы, на выгодоприобретателя (ст. 18 Закона).</p>
            <p>
              В случае отказа в выдаче страховых вы плат либо нарушений условий договора обязательного страхования
              туриста, обратится с заявлением и прилагаемыми документами страховому омбудсмену напрямую через интернет
              ресурсы, либо через страховщика.
              Срок выплаты страховых выплат – 15 рабочих дней со дня получения им документов . В случае оспаривания
              части страховых выплат , должна быть выплачена в течении трех рабочих дней со дня вступления решения суда
              по данному спору, если судом решение не обращено немедленному исполнению (п.7 ст. 18 закона).
            </p>
          </li>
          <li>
            <p>
              Обратится в суд за защитой своих законных прав согласно установленного процессуального законодательства ,
              Закона о страховой деятельности, Закона об обязательном страховании туриста.
            </p>
          </li>
        </ol>
      </div>`,
      contentKk: `<div class="modal__text-info">
        <ol>
          <li>
            <p>
              Сақтандыру жағдайының басталғаны туралы сақтандырушыны және уақытша болатын елдің басқа да уәкілетті билік органдарын дереу өзі немесе өкіл арқылы хабардар етуге және ұсыныстарды орындауы тиіс (Туристі міндетті сақтандыру туралы заңның (бұдан әрі – Заң) 12-бабының 2-1-тармағы).
            </p>
            <div class="tooltip-wrap">
                 <div class="tooltip-btn" data-tooltip-id="my-tooltip" data-tooltip-variant="info" data-tooltip-place="right-start" data-tooltip-content="Туристік сапарға құжаттарды алған кезде міндетті түрде орындаушидан сақтандыру сертификаты мен сақтандыру полисін ұсыну талап етілуі қажет.">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 22C17.5 22 22 17.5 22 12C22 6.5 17.5 2 12 2C6.5 2 2 6.5 2 12C2 17.5 6.5 22 12 22Z" fill="white"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M12 7.25C12.4142 7.25 12.75 7.58579 12.75 8V13C12.75 13.4142 12.4142 13.75 12 13.75C11.5858 13.75 11.25 13.4142 11.25 13V8C11.25 7.58579 11.5858 7.25 12 7.25Z" fill="#D5251B"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M10.9945 16C10.9945 15.4477 11.4422 15 11.9945 15H12.0035C12.5558 15 13.0035 15.4477 13.0035 16C13.0035 16.5523 12.5558 17 12.0035 17H11.9945C11.4422 17 10.9945 16.5523 10.9945 16Z" fill="#D5251B"/>
                  </svg>
                </div>
            </div>
          </li>
          <li>
            <p>Сақтандырушыға денсаулыққа келтірілген зиянның сипаты мен мөлшері туралы мән-жайларды анықтау үшін қажетті қолда бар құжаттарды ұсыну (Заңның 12-бабының 2-1-тармағы) және 5 жұмыс күні ішінде сақтандырушы сақтандыру төлемінің мөлшерін анықтауға және танысуға ұсынуға тиіс (Заңның 13-бабының 2-тармағының 4-3-тармағы). Соның ішінде, егер олар қажет болса және басталған нәтижеге қарамастан сақтандырушының нұсқауларын орындау үшін жүргізілген болса, шығындарды болдырмау немесе азайту мақсатында шеккен шығыстарды өтеуді талап етіледі (Заңның 14-бабының 3-тармағы).

              Қажетті құжаттардың тізбесі: жеке басын куәландыратын құжат немесе шекара қызметінің белгісі бар сақтандырылушының паспорты; түпнұсқасы немесе бал. медициналық қызметке ақы төлеу, тауарлар мен қызметтерге ақы төлеу туралы төлем құжаттары (дәрі-дәрмектерді және өзге де медициналық қызметтерді сатып алу және т.б.), жазатайым оқиға басталған кезде сақтандырылған адамның психикалық белсенді пайдалануына және мас күйіне қатысты медициналық құжаттар;
              
              жол жүру құжаттарының күшін жоюды немесе айырбастауды, қоршалған қонақ үй нөмірінен және басқа қызметтерден бас тартуды растайтын құжаттар, оның ішінде кәмелетке толмағандарды тасымалдау кезінде;
сақтандырушыға телефон қоңырауларының, хабарламаларының жүзеге асырылғанын растайтын құжаттар;
шығындарды болдырмау және азайту жөнінде шаралар қабылдауға байланысты құжаттар;
пайда алушыға арналған құжаттар (Заңның 18-бабы).
              </p>
            <p>
              Сақтандыру төлемдерін беруден бас тартқан не туристі міндетті сақтандыру шартының талаптарын бұзған жағдайда, сақтандыру омбудсменіне тікелей интернет ресурстары арқылы не сақтандырушы арқылы өтініш және қоса берілген құжаттар беріледі. Сақтандыру төлемдерін төлеу мерзімі - құжаттарды алған күннен бастап 15 жұмыс күні. Сақтандыру төлемдерінің бір бөлігіне дау айтылған жағдайда, егер сот шешімді дереу орындамаса, осы дау бойынша сот шешімі күшіне енген күннен бастап үш жұмыс күні ішінде төленуге тиіс (заңның 18-бабының 7-тармағы).
            </p>
          </li>
          <li>
            <p>
              Белгіленген іс жүргізу заңнамасына,
              Сақтандыру қызметі туралы заңға,
              Туристі міндетті сақтандыру туралы заңға сәйкес өзінің заңды құқықтарын қорғау үшін сотқа жүгіну.
            </p>
          </li>
        </ol>
      </div>`,
    },
    {
      id: 4,
      text: 'Утеря документов',
      textKk: 'Құжаттардың жоғалуы',
      ispPrevInfo: true,
      content: `<div class="modal__text-info">
        <ol>
          <li>
            <p>
              В случае кражи или утери документов, удостоверяющих личность (удостоверение личности, паспорт), обратится с заявлением в полицию по месту временного пребывания. Получить справку - подтверждение факта потери документа.
            </p>
          </li>
          <li>
            <p>
            Обратится в дипломатические представительства или консульские учреждения Республики Казахстан (п. 2 ст. 21 Закона о документах удостоверяющих личность, далее - Закон) для получения свидетельства на возвращение.</p>
            <div class="tooltip-wrap">
              <div class="tooltip-btn" data-tooltip-id="my-tooltip" data-tooltip-variant="info" data-tooltip-place="right-start" data-tooltip-content="Одновременно с вышеуказанными действиями, в обязательном порядке сообщить турагентству и принимающей стороне об утере документов.">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 22C17.5 22 22 17.5 22 12C22 6.5 17.5 2 12 2C6.5 2 2 6.5 2 12C2 17.5 6.5 22 12 22Z" fill="white"/>
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M12 7.25C12.4142 7.25 12.75 7.58579 12.75 8V13C12.75 13.4142 12.4142 13.75 12 13.75C11.5858 13.75 11.25 13.4142 11.25 13V8C11.25 7.58579 11.5858 7.25 12 7.25Z" fill="#D5251B"/>
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M10.9945 16C10.9945 15.4477 11.4422 15 11.9945 15H12.0035C12.5558 15 13.0035 15.4477 13.0035 16C13.0035 16.5523 12.5558 17 12.0035 17H11.9945C11.4422 17 10.9945 16.5523 10.9945 16Z" fill="#D5251B"/>
                </svg>
              </div>
            </div>
            <ul>
                <li>
                    <p>
                        Свидетельство на возвращение выдается в случае утраты паспорта или истечения срока его действия во время пребывания за пределами РК. В него вносятся отметки о визах и пересечении государственнойграницы аналогично паспорту (ст. 17 Закона).
                    </p>
                </li>
                <li>
                    <p>
                        Утраченный документ удостоверяющий личность производится по месту жительства в общем порядке согласно Закону.
                    </p>
                </li>
            </ul>
          </li>
          <li>
            <p>
              Представляемые документы для получения свидетельства на возвращение: справка – подтверждение факта кражи или утери паспорта, выданное полицейским учреждением, при наличии удостоверение личности или другой документ, подтверждающий личность заявителя; документ, подтверждающий консульские сборы.
            </p>
          </li>
        </ol>
        <div class="modal__text-title">
            Рекомендации:
        </div>
        <ol>
          <li>
            <p>
              до конца тура сохранять договор на туристское обслуживание; 
            </p>
          </li>
          <li>
            <p>
              При заключении договора на туристское обслуживание  требовать следующие документы: договор в одном экз. на русском и казах.языках, турист.ваучер, тур.код,  страховой сертификат и страховой полис;
            </p>
          </li>
          <li>
            <p>
              Обратится в КФ «Туристік Қамқор» (www.fondkamkor.kz) за юридической или фактической поддержки.
            </p>
          </li>
        </ol>
      </div>`,
      contentKk: `<div class="modal__text-info">
        <ol>
          <li>
            <p>
             Жеке басын куәландыратын құжаттар (жеке куәлік, паспорт) ұрланған немесе жоғалған жағдайда уақытша тоқтаған жері бойынша полицияға өтініш береді. Анықтама алуы – құжатты жоғалту фактісін растайды.
            </p>
          </li>
          <li>
            <p>Қайтып оралуға арналған куәлікті алу үшін Қазақстан Республикасының дипломатиялық өкілдіктеріне немесе консулдық мекемелеріне (Жеке басты куәландыратын құжаттар туралы заңның 21-бабының 2-тармағы, бұдан әрі – Заң) жүгіну.</p>
            <div class="tooltip-wrap">
              <div class="tooltip-btn" data-tooltip-id="my-tooltip" data-tooltip-variant="info" data-tooltip-place="right-start" data-tooltip-content="Жоғарыда көрсетілген іс-қимылдармен бір мезгілде міндетті түрде турагенттікке және қабылдаушы тарапқа құжаттардың жоғалғаны туралы хабарлау.">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 22C17.5 22 22 17.5 22 12C22 6.5 17.5 2 12 2C6.5 2 2 6.5 2 12C2 17.5 6.5 22 12 22Z" fill="white"/>
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M12 7.25C12.4142 7.25 12.75 7.58579 12.75 8V13C12.75 13.4142 12.4142 13.75 12 13.75C11.5858 13.75 11.25 13.4142 11.25 13V8C11.25 7.58579 11.5858 7.25 12 7.25Z" fill="#D5251B"/>
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M10.9945 16C10.9945 15.4477 11.4422 15 11.9945 15H12.0035C12.5558 15 13.0035 15.4477 13.0035 16C13.0035 16.5523 12.5558 17 12.0035 17H11.9945C11.4422 17 10.9945 16.5523 10.9945 16Z" fill="#D5251B"/>
                </svg>
              </div>
            </div>
            <ul>
                <li>
                    <p>
                        Қайтып оралуға арналған куәлік паспорты жоғалған немесе оның қолданылу мерзімі аяқталған жағдайда ҚР шегінен тыс жерде болған уақытта беріледі. Оған паспортқа ұқсас визалар мен мемлекеттік шекараны кесіп өту туралы белгілер енгізіледі (Заңның 17-бабы).
                    </p>
                </li>
                <li>
                    <p>
                        Жоғалған жеке басын куәландыратын құжат Заңға сәйкес тұрғылықты жері бойынша жалпы тәртіппен жүргізіледі.
                    </p>
                </li>
            </ul>
          </li>
          <li>
            <p>
              Қайтып оралуға арналған куәлікті алу үшін ұсынылатын құжаттар: полиция мекемесі берген паспортты ұрлау немесе жоғалту фактісін растайтын анықтама, жеке куәлік немесе өтініш берушінің жеке басын растайтын басқа құжат болған жағдайда; консулдық алымдарды растайтын құжат.
            </p>
          </li>
        </ol>
        <div class="modal__text-title">
            ҰСЫНЫСТАР:
        </div>
        <ol>
          <li>
            <p>
              турдың соңына дейін туристік қызмет көрсетуге арналған шартты сақтау;
            </p>
          </li>
          <li>
            <p>
              Туристік қызмет көрсетуге шарт жасасу кезінде мынадай құжаттар талап ету: орыс және қазақ тілдерінде бір данада шарт, туристік ваучер, туркод, сақтандыру сертификаты және сақтандыру полисі;
            </p>
          </li>
          <li>
            <p>
              «Туристік Қамқор» ҚФ (www.fondkamkor.kz) заңды немесе іс жүзінде қолдау үшін жүгіну.
            </p>
          </li>
        </ol>
      </div>`,
    }
  ]

  const openModal = (id, ispPrevInfo, text, textKk, content, contentKk) => {
    if(!ispPrevInfo) {
      const newObj = {
        id: id,
        title: text,
        titleKk: textKk,
      }
      setInData(newObj)
      router.push(`/help/in?title=${id}`)
    } else {
      toggleModal()
      const newObj = {
        text: text,
        textKk: textKk,
        content: content,
        contentKk: contentKk
      }
      setData(newObj)
    }

  }

  return (
      <div className='page-blank__container'>
        <div className="container">
          <div className="page-blank">
            <HeadPage title={translations[locale].problemsTour}/>
            <div className="help-list__wrap">
              <ul className="help-list list-reset">
                {
                    helpVariants && helpVariants.map(({id, text, textKk, ispPrevInfo, content, contentKk}) => {
                      return (
                          <li key={id} className='help-list__item' onClick={() => openModal(id, ispPrevInfo, text, textKk, content, contentKk)}>
                          <span>
                            {locale === 'ru' ? text : textKk}
                          </span>
                            <svg
                                width="24"
                                height="24"
                                viewBox="0 0 24 24"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                              <path
                                  d="M14.4297 5.92999L20.4997 12L14.4297 18.07"
                                  stroke='var(--color-primary)'
                                  strokeWidth="1.5"
                                  strokeMiterlimit="10"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                              />
                              <path
                                  d="M3.5 12H20.33"
                                  stroke='var(--color-primary)'
                                  strokeWidth="1.5"
                                  strokeMiterlimit="10"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                              />
                            </svg>
                          </li>
                      )
                    })
                }
              </ul>
            </div>
          </div>
        </div>
      </div>
  )
}